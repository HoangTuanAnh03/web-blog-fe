...
import { Menu, User, LogOut, PenSquare, Compass, Bot, Rss } from "lucide-react";
...
export function SiteHeader() {
  ...
  const linkClass = (href: string) =>
    cn(
      "relative inline-flex items-center rounded-md px-1.5 py-1 text-sm font-autopost transition-colors",
  ...
  useEffect(() => {
    try {
      const seen = localStorage.getItem("feature:autopostFollowSeen");
      setShowNewBadge(!seen);
    } catch {}
  }, []);
                      ...
                      {isAuthenticated ? (
                        ...
                        <div className="flex gap-2">
                          <Link href="/login" className="flex-1">
                            <Button
                              variant="ghost"
                              className="w-full hover:bg-accent font-autopost"
                            >
                              Đăng Nhập
                            </Button>
                          </Link>
                          <Link href="/signup" className="flex-1">
                            <Button className="w-full rounded-full shadow-card font-autopost">
                              Đăng Ký
                            </Button>
                          </Link>
                        </div>
            ...
            <div className="flex items-center gap-1 md:gap-2">
              {isAuthenticated ? (
                <>
                  ...
                  <Link href="/auto-post" className="md:hidden">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="rounded-full hover:bg-accent focus-visible:ring-2 focus-visible:ring-primary/40"
                      aria-label="Tạo bài viết tự động"
                    >
                  ...
                  <Link href="/auto-post" className="hidden md:block">
                    <Button
                      variant="outline"
                      className="gap-2 rounded-full shadow-card focus-visible:ring-2 focus-visible:ring-primary/40 relative"
                      onClick={() => {
                        try {
                          localStorage.setItem("feature:autopostFollowSeen", "1");
                        } catch {}
                        setShowNewBadge(false);
                      }}
                      aria-label="Mở trang Tạo bài viết tự động"
                    >
                      <Bot className="h-4 w-4" />
                      Tự động đăng
                      {showNewBadge && (
                        <span className="ml-2 rounded-full bg-primary/15 text-primary text-[10px] px-2 py-0.5">
                          Mới
                        </span>
                      )}
                    </Button>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem asChild>
                        <Link href="/auto-post" className="cursor-pointer">
                ...
                <div className="flex items-center gap-1 md:gap-2">
                  <Link href="/login">
                    <Button className="rounded-full font-autopost shadow-card focus-visible:ring-2 focus-visible:ring-primary/40">
                      Đăng Nhập
                    </Button>
                  </Link>
                  <Link href="/signup">
                    <Button
                      variant="outline"
                      className="rounded-full font-autopost focus-visible:ring-2 focus-visible:ring-primary/40"
                    >
                      Đăng Ký
                    </Button>
                  </Link>
