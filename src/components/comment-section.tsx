...
export function CommentSection({ blogId, comments: initialComments }: CommentSectionProps) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch("https://api.sportbooking.site/blog/comment", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${JSON.parse(window.localStorage.getItem('authState') || '{}')?.accessToken || ""}`
        },
        body: JSON.stringify({
                    ...
                    {isAuthenticated && (
                      <Button
                        ...
                        onClick={() => setReplyingTo(
                          replyingTo === String(comment.id) ? null : String(comment.id)
                        )}
                ...
                {comment.replies && comment.replies.length > 0 && (
                  <div className="ml-14 space-y-4">
                    {(expandedReplies[String(comment.id)]
                      ...
                      <div key={reply.id} className="flex gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
                        <Avatar className="h-8 w-8">
                          <AvatarImage
            ...
            {comments.length > visibleComments && (
              <div className="flex justify-center pt-4">
                <Button 
                  ...
                  className="px-8"
                >
                  Xem thêm bình luận ({comments.length - visibleComments} còn lại)
                </Button>
              </div>
            )}
          </div>
